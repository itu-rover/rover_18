...
def letsSerial():
    ...
    while not rospy.is_shutdown():
        ...
        ser.timeout = 1
        ser2 = serial.Serial(port='/dev/ttyUSB0', baudrate=9600, parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE,
                            bytesize=serial.EIGHTBITS)  # open serial
        ser2.timeout = 1


        rospy.loginfo(ser.portstr)
        rospy.loginfo(ser2.portstr)

        while ser.isOpen() and ser2.is_open():
            if printOnce == True:
