...
def waypointSubscriber(WPMsg): #Waypoint Command recieved from ROS topic, run this function
   ...
   # print("burda")
    ...
    WPUpdateState = True
    latWP = WPMsg.longitude
    lonWP = WPMsg.latitude
    altWP = WPMsg.altitude
...
def main():
    ...
    while not rospy.is_shutdown(): #While ros comms are running smoothly
        rospy.Subscriber("/rover_gps/waypoint", NavSatFix, waypointSubscriber) #Subscribe to "pose", "fix" and "waypoint" ROS topics
